...
import { getSupabase, exerciseId, variationExerciseId } from '@gym-tracker/core';
...
export function toHistorySlotEntries(history: LoggedWorkout[]): HistorySlotEntry[] {
  ...
  for (const w of history) {
    ...
    for (const s of w.slots) {
      ...
      const slotId = exerciseId(s.slot);
      const exId = variationExerciseId(s.slot, s.kind) ?? slotId;
      out.push({ slot: s.slot, slotId, exerciseId: exId, kind: s.kind, date: w.date, sets: s.sets });
    }
  }
  return out;
}
